function func_visualize_ND_slicing(fig_id, draw_link_line, draw_link_probability, draw_link_intensity, draw_link_count, ...
                                   draw_strongest_link_only, draw_link_proportional)

%% visualization C-mode slicing
x_axis_dim_idx
y_axis_dim_idx

%% specify range for the other dimensions

global state_color_list;
...
global Sr_truev_entry_list;
global Sr_truev_exit_list;
global subject_list;
...
for i = 1:data_setting.dim_state_num_ls(1) % function 
    for j = 1:data_setting.dim_state_num_ls(2) % structure               
            ...
            %% draw number of entry, exit
            num_subject_entry = size(Sr_truev_entry_list{s}.subject_id_list, 2);
            num_subject_exit = size(Sr_truev_exit_list{s}.subject_id_list, 2);
            
            entry_marker_size = min_marker_size + double(num_subject_entry) / double(max_num_subject) * double(max_marker_size-min_marker_size);
            exit_marker_size = min_marker_size + double(num_subject_exit) / double(max_num_subject) * double(max_marker_size-min_marker_size);
            
            if (num_subject_truev > 0)                
                if (entry_marker_size > exit_marker_size)                    % draw entry then exit
                   color = state_color_list(1, :);
                   plot(draw_i,draw_j,'ro','MarkerEdgeColor','k','MarkerFaceColor',color,'MarkerSize', entry_marker_size);                    
                   if (num_subject_exit > 0)
                       color = state_color_list(2, :);
                       plot(draw_i,draw_j,'ro','MarkerEdgeColor','k','MarkerFaceColor',color,'MarkerSize', exit_marker_size); 
                   end
                elseif (entry_marker_size < exit_marker_size)                    
                   % draw exit then entry
                   color = state_color_list(2, :);
                   plot(draw_i,draw_j,'ro','MarkerEdgeColor','k','MarkerFaceColor',color,'MarkerSize', exit_marker_size); 
                   if (num_subject_entry > 0)
                       color = state_color_list(1, :);
                       plot(draw_i,draw_j,'ro','MarkerEdgeColor','k','MarkerFaceColor',color,'MarkerSize', entry_marker_size);                        
                   end                    
                else %'==' 
                   color = (state_color_list(1, :) + state_color_list(2, :));
                   plot(draw_i,draw_j,'ro','MarkerEdgeColor','k','MarkerFaceColor',color,'MarkerSize', exit_marker_size);                      
                end
                                
            elseif (num_subject_interpov > 0)
            ...
            %% draw number of exit            
            label = sprintf('(%d,%d),%d', num_subject_entry, num_subject_exit, num_subject_interpov); 
        ...
        for n = 1:num_state
            ...
            if (Q_mat_struct(m, n) == 1 && Nrs_mat(m, n) > 0)            
                ...
                if ((draw_strongest_link_only == 1 && max_local_link == Q_mat(m, n)) || ...
                    ...
                    text(x, y, label, 'Color', text_color, 'FontSize',linkfontsize);
