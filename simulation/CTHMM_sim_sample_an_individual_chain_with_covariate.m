function chain = CTHMM_sim_sample_an_individual_chain_with_covariate(sampling_dur)

global sample_chain_Q_mat;
global state_init_prob_list;
global syn_covariate_w_list;

%% sample a (state,duration,covariate,Q) chain, such that the total duration is >= (obs_total_dur + (t1 - t0))

num_state = size(sample_chain_Q_mat, 1);
chain.state_idx_list = zeros(1, num_state);
chain.state_dur_list = zeros(1, num_state);
chain.state_cov_list = cell(num_state, 1);
chain.accum_state_dur_list = zeros(1, num_state);
accum_state_dur = 0.0;
num_sampled_state = 0;

%% covariate
num_cov = size(syn_covariate_w_list, 1);


[rand_cov_list, sample_ind_Q_mat] = CTHMM_sim_sample_covariate_ind_Q(num_cov);

while (accum_state_dur < sampling_dur)
    
    %% sample covariates and compute current individualized Q mat
    %[rand_cov_list, sample_ind_Q_mat] = CTHMM_sim_sample_covariate_ind_Q(num_cov);
    ...
    if (num_sampled_state == 0) 
        ...
        temp_row = sample_ind_Q_mat(prev_state, :);
        nb_idx_ls = find(temp_row > 0);
        
        %% sample the state
        q_list = temp_row(nb_idx_ls); 
        prob_list = q_list ./ (-sample_ind_Q_mat(prev_state, prev_state));
    ...
    if (CTHMM_get_num_of_outgoing_neighbor(sample_ind_Q_mat, cur_state) == 0) % if cur state is an absorption state                        
        ...
        qi = sample_ind_Q_mat(cur_state, cur_state);
    ...
    %chain.ind_Q_list(i) = sample_ind_Q_mat;
    chain.state_cov_list{i} = rand_cov_list;
...
chain.state_cov_list = chain.state_cov_list(1:num_sampled_state);
